...
const ImageCapa: React.FC = () => {
  ...
  return (
    <div className={`mt-2 relative h-[${divHeight}] `} // Utiliza a classe condicional para definir a altura
        ...
        {image && (
          <div className='w-full relative h-[260px]'>
            <img src={image} alt="Imagem adicionada" className=' rounded w-full h-full' /> 
            {showCoverButton && (
        ...
        {showCoverButton && (
          <div className='absolute bottom-0 mb-2'>
            {!image && (
            <label className='flex rounded py-1 px-2 bg-neutral-700 items-center gap-1 cursor-pointer text-zinc-200 text-[12px]' htmlFor="cover"><Image className="text-zinc-200 cursor-pointer" size={18} />Add cover</label>
            )}
            <Input id="cover" name="cover" className='mt-2 bg-neutral-700 hidden' type="file" accept="image/*" onChange={handleImageChange} />
          </div>
        )}
